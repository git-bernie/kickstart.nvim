...
-- Wrapped in functions to defer MiniMap lookup (mini.map loads via VeryLazy)
vim.keymap.set('n', '<Leader>mc', function()
  MiniMap.close()
end, { desc = '[m]inimap [c]lose' })
vim.keymap.set('n', '<Leader>mf', function()
  MiniMap.toggle_focus()
end, { desc = '[m]inimap [f]ocus' })
vim.keymap.set('n', '<Leader>mo', function()
  MiniMap.open()
end, { desc = '[m]inimap [o]pen' })
vim.keymap.set('n', '<Leader>mr', function()
  MiniMap.refresh()
end, { desc = '[m]inimap [r]efresh' })
vim.keymap.set('n', '<Leader>ms', function()
  MiniMap.toggle_side()
end, { desc = '[m]inimap [s]ide' })
vim.keymap.set('n', '<Leader>mt', function()
  MiniMap.toggle()
end, { desc = '[m]inimap [t]oggle' })
...
-- Copies to "+" (system clipboard) and "g register. Use :Pwd! for non-~ (absolute) path.
vim.api.nvim_create_user_command('Pwd', function(opts)
  ...
  local msg = leaf .. '/\n' .. display_path .. '\n("+ and "g = ' .. path_type .. ')'
  vim.notify(msg, vim.log.levels.INFO)
end, { bang = true, desc = 'Show cwd (leaf first), yank to "+ and "g. Use ! for absolute.' })
